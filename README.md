for website:
- open codecoach/codecoach-web
- npm run dev
- open local host
- log in as student demo: student@u.northwestern.edu
- log in as instructor demo: instructor@u.northwestern.edu

for codecoach IDE:
- open codecoach/codecoach-vscode-extension/codecoach
- npm run compile
- start debugging (Run dropdown and press start debugging or F5)
- should open Extension Development Host
- open file in Extension Development Host
- press square on bottom of left sidebar for chat
    - if not there, do cmnd + shift + p and type in CodeCoach: Open Chat
- press connect to connect to account (student@u.northwestern.edu)
    - get student CodeCoach token from settings in web app
- then can open up assignment
